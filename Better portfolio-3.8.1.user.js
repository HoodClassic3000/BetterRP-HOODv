...
(function () {
  'use strict';
    ...
    const HOPIUM_API_PATH = 'api/hopium';
    const TRUNCATED_COUNT = 0;
  // --- URL logic ---
  function isPortfolioPage() {
    return location.hostname.endsWith("rugplay.com") &&
      location.pathname === "/portfolio";
  }
    function isHopiumPage() {
    return location.hostname.endsWith("rugplay.com") &&
      location.pathname === "/hopium";
  }
      function onCoinPage() {
        return /^\/coin\/[^/]+$/.test(window.location.pathname);
    }
      // --- UTILITIES ---
  ...
  const coinAmountFromValue = (price, value) => {
  const k = 1_000_000_000_000;
  const C = Math.sqrt(k / price);
  const B = k / C;
  return (k / (B - value)) - C;
};
  if (!window.__portfolioFetchOverridden) {
      window.__portfolioFetchOverridden = true;
        // --- OVERRIDE FETCH FOR PORTFOLIO API PATCH ---
    const origFetch = window.fetch;
    window.fetch = async function (input, init) {
      const url = typeof input === 'string' ? input : input.url;
      if (url.includes(PORTFOLIO_API_PATH) && isPortfolioPage()) {
...
if (url.includes(HOPIUM_API_PATH) && isHopiumPage()) {
  const resp = await origFetch.apply(this, arguments);
  const clone = resp.clone();
  const data = await clone.json();

  if (Array.isArray(data.questions)) {
    // Remove questions where creator name is null
    const patchedQuestions = data.questions.filter(
      q => q.creator && q.creator.name !== null
    );

    // You can optionally assign or process patchedQuestions here

    // Forward the modified response (with only valid questions)
    const patchedData = { ...data, questions: patchedQuestions };
  ...
  // --- MAIN SCRIPT: Only runs on /portfolio ---
...
const renderRows = () =>
  filteredDataSet.map((row) => {
    ...
    return `<tr data-slot="table-row" class="data-[state=selected]:bg-muted border-b hover:bg-muted/50 cursor-pointer transition-colors" style="cursor:pointer"
      ...
      cols
        .map((col, i) => {
          const isTokenCol = col === 'symbol';
          const cellValue = row[col] ?? '';
          return `<td data-slot="table-cell" class="whitespace-nowrap p-2 align-middle font-medium text-xs ${
            isRenderableNumber(row[col]) && !isTokenCol ? 'font-mono' : ''
          } ${colDefs[i].class}" title="${cellValue}">` +
            (!isTokenCol && isRenderableNumber(row[col]) && Math.abs(Number(row[col])) >= 1000
              ? formatNumberSmart(row[col])
              : !isTokenCol && isRenderableNumber(row[col])
              ? formatNumberSmart(row[col])
              : cellValue) +
            `</td>`;
        })
    ...
    function initClusterizeWhenReady() {
      // Try to get the elements by ID
      const scrollEl = document.getElementById(scrollId);
      const contentEl = document.getElementById(contentId);
      if (!scrollEl || !contentEl) {
        // Try again on next animation frame
        requestAnimationFrame(initClusterizeWhenReady);
        return;
      }
  ...
  // Toggle logic
  let coinInputMode = 'amount'; // 'amount' or 'value'
  ...
  // Invert AMM: calculate coin quantity from desired value
  function ammQuantityFromValue(price, value) {
    // If value is not positive, return 0
    if (!price || !value || value <= 0) return 0;
    const k = 1_000_000_000_000;
    const C = Math.sqrt(k / price);
    const B = k / C;
    // Solve for quantity: B - k/(C + q) = value
    // B - value = k/(C+q)
    // C+q = k/(B-value)
    // q = k/(B-value) - C
        ...
        <div id="rp-sendmoney-coinblock" style="margin-bottom:12px;display:none;">
          ...
          <div style="margin-top:10px;">
            <label style="font-weight:500;">
              Enter <span id="rp-sendmoney-toggle-label">coin amount</span>
              <span id="rp-sendmoney-toggle-wrap" style="margin-left:12px;">
                <button id="rp-sendmoney-toggle" type="button" style="font-size:13px;background:#232334;border-radius:4px;border:1px solid #383c4a;color:#eee;padding:3px 10px;cursor:pointer;">Switch to value</button>
              </span>
            </label>
          </div>
        </div>
  ...
  const coinBlockToggleBtn = modal.querySelector('#rp-sendmoney-toggle');
  const coinBlockToggleLabel = modal.querySelector('#rp-sendmoney-toggle-label');
  const coinBlockToggleWrap = modal.querySelector('#rp-sendmoney-toggle-wrap');
  ...
  function filterCoinsForSearch(query) {
    ...
    return coinHoldings.filter((h) => fuzzyMatchCoin(h.symbol, query));
  }
  ...
  // Toggle between "amount" and "value" input modes
  function updateCoinBlockToggle() {
    if (transferType !== 'COIN') {
      coinBlockToggleWrap.style.display = 'none';
      coinBlockToggleLabel.textContent = '';
      return;
    }
    coinBlockToggleWrap.style.display = '';
    if (coinInputMode === 'amount') {
      coinBlockToggleLabel.textContent = 'coin amount';
      coinBlockToggleBtn.textContent = 'Switch to value';
      amountInput.step = '0.000001';
      amountInput.placeholder = '0.000000';
    } else {
      coinBlockToggleLabel.textContent = 'actual value';
      coinBlockToggleBtn.textContent = 'Switch to amount';
  ...
  coinBlockToggleBtn.addEventListener('click', () => {
    coinInputMode = coinInputMode === 'amount' ? 'value' : 'amount';
    updateCoinBlockToggle();
  });
  ...
  function updateAvailableInfo() {
    ...
    let numericAmount = parseFloat(amountInput.value) || 0;
    let displayCoinAmount = numericAmount;
    let usedValue = 0;
    let displayValue = 0;

    // If coin mode and "value" input, recalculate amount
    if (
      transferType === 'COIN' &&
      selectedHolding &&
      numericAmount > 0 &&
      ...
      // User entered target value, calculate coin quantity to send via AMM
      displayCoinAmount = ammQuantityFromValue(selectedHolding.currentPrice, numericAmount);
      numericAmount = displayCoinAmount;
      // What is the "server" value for this amount? (quantity * currentPrice)
      usedValue = displayCoinAmount * selectedHolding.currentPrice;
      // True AMM value
      displayValue = coinValueCalcWithoutK(selectedHolding.currentPrice, displayCoinAmount);
    }
    ...
    if (transferType === 'COIN' && selectedHolding && numericAmount > 0) {
      let serverEstValue, trueEstValue;

      if (coinInputMode === 'value') {
        // Input is value, coin calculated above
        serverEstValue = usedValue;
        trueEstValue = displayValue;
        coinValueSpan.textContent = `Server Estimated Value: ≈ $${serverEstValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (amount: ${displayCoinAmount.toFixed(6)} ${selectedCoinSymbol})`;
      } else {
        // Input is coin amount
        serverEstValue = numericAmount * selectedHolding.currentPrice;
        trueEstValue = coinValueCalcWithoutK(selectedHolding.currentPrice, numericAmount);
  ...
  function updateCoinInputs() {
    ...
    if (transferType === 'COIN') {
      ...
      amtLabel.textContent =
        coinInputMode === 'amount'
          ? `Amount (${selectedCoinSymbol || '...'})`
          : `Amount ($ actual value)`;
      updateCoinBlockToggle();
      ...
      amountInput.step = '0.01';
      amountInput.placeholder = '0.00';
      coinInputMode = 'amount'; // fallback to amount for cash
    }
    updateAvailableInfo();
    validateAll();
  }
  ...
  typeSel.addEventListener('change', () => {
    ...
    coinInputMode = transferType === 'COIN' ? coinInputMode : 'amount';
    amountInput.value = '';
    updateCoinInputs();
  });
  ...
  maxBtn.addEventListener('click', () => {
    ...
    if (transferType === 'COIN' && coinInputMode === 'value' && selectedHolding) {
      // In value mode, max value is AMM value for user's quantity
      maxAmount = coinValueCalcWithoutK(selectedHolding.currentPrice, selectedHolding.quantity);
    }
  ...
  const validateAll = () => {
    ...
    // If coin mode and value, get coin amount for given value
    if (
      transferType === 'COIN' &&
      selectedHolding &&
      numericAmount > 0 &&
      ...
      numericAmount = ammQuantityFromValue(selectedHolding.currentPrice, numericAmount);
    }
    ...
    // The minimum value check should STILL use server calculation: coinAmount * price
    ...
    if (!hasEnoughFunds && hasValidAmount) {
      ...
      minMsg.textContent = `Coin transfers require a minimum estimated value of $10.00 (server calculation)`;
      minMsg.style.display = '';
    }
  ...
  modal.querySelector('#rp-sendmoney-form').addEventListener('submit', async (e) => {
    ...
    const recipient = modal.querySelector('#rp-sendmoney-recipient').value.trim();
    let inputValue = parseFloat(amountInput.value) || 0;
    const selectedHolding = getCoinHolding(selectedCoinSymbol);

    // If coin mode and value, convert value to coin amount before submit
    let sendAmount = inputValue;
    if (
      transferType === 'COIN' &&
      selectedHolding &&
      inputValue > 0 &&
      ...
      sendAmount = ammQuantityFromValue(selectedHolding.currentPrice, inputValue);
    }
    ...
    const hasEnoughFunds =
      transferType === 'CASH'
        ? sendAmount <= userBalance
        : selectedHolding
        ? sendAmount <= selectedHolding.quantity
        : false;
    const estimatedValue =
      transferType === 'COIN' && selectedHolding && sendAmount > 0
        ? sendAmount * selectedHolding.currentPrice
        : transferType === 'CASH'
        ? sendAmount
        : 0;
    const isWithinCashLimit = transferType === 'CASH' ? sendAmount >= 10 : true;
...
} // End runPortfolioScript()

function tradeModalModification(){


	const POOL_ENDPOINT = (symbol) =>
		`https://rugplay.com/api/coin/${encodeURIComponent(symbol)}/holders?limit=1`;

	let lastModal = null;
	let poolCache = new Map();
	let inflight = new Map();
  // Place after: let poolCache = new Map(); let inflight = new Map();
let ws;
function initPriceSocket() {
	if (ws) return;
	ws = new WebSocket('wss://ws.rugplay.com/');
	ws.addEventListener('message', (event) => {
		try {
			const data = JSON.parse(event.data);
			if (data?.type !== 'price_update' || !data?.coinSymbol) return;

			const symbol = data.coinSymbol.toUpperCase();
			poolCache.set(symbol, {
				coinAmount: Number(data.poolCoinAmount) || 0,
				baseCurrencyAmount: Number(data.poolBaseCurrencyAmount) || 0,
				currentPrice: Number(data.currentPrice) || 0
			});

			// If the trade modal is open for this coin, re-sync inputs
			const modal = document.querySelector('[data-slot="dialog-content"]') || document.querySelector('[role="dialog"]');
			if (modal) {
				const title = modal.querySelector('[data-slot="dialog-title"]') || modal.querySelector('h2');
				const match = (title?.textContent || '').match(/(BUY|SELL)\s+([A-Z0-9]+)/i);
				const modalSymbol = match ? match[2].toUpperCase() : null;
				if (modalSymbol === symbol) {
					const valueInput = modal.querySelector('[data-dual-input="value"] input');
					if (valueInput) valueInput.dispatchEvent(new Event('input', { bubbles: true }));
				}
			}
		} catch {
	...
	ws.addEventListener('close', () => {
		ws = null;
		// simple reconnect
		setTimeout(initPriceSocket, 2000);
	});
}
initPriceSocket();
	...
	async function getPoolInfo(symbol) {
		if (poolCache.has(symbol)) return poolCache.get(symbol);
		if (inflight.has(symbol)) return inflight.get(symbol);

		const p = fetch(POOL_ENDPOINT(symbol), { credentials: 'include' })
			.then((res) => res.json())
			.then((data) => {
				const pool = data?.poolInfo || {};
				const result = {
					coinAmount: sanitizeNumber(pool.coinAmount),
					baseCurrencyAmount: sanitizeNumber(pool.baseCurrencyAmount),
					currentPrice: sanitizeNumber(pool.currentPrice)
				};
				poolCache.set(symbol, result);
	...
	function ammEstimateValueFromCoins(coins, poolCoin, poolBase) {
		if (!coins || poolCoin <= 0 || poolBase <= 0) return 0;
		const k = poolCoin * poolBase;
		const newPoolCoin = poolCoin + coins;
		const newPoolBase = k / newPoolCoin;
		return poolBase - newPoolBase;
	}

	function ammEstimateCoinsFromValue(value, poolCoin, poolBase) {
		if (!value || poolCoin <= 0 || poolBase <= 0) return 0;
		const k = poolCoin * poolBase;
		const newPoolBase = poolBase + value;
		const newPoolCoin = k / newPoolBase;
		return poolCoin - newPoolCoin;
	}
function ammEstimateCoinsToSellForValue(value, poolCoin, poolBase) {
	if (!value || poolCoin <= 0 || poolBase <= 0 || value >= poolBase) return 0;
	const k = poolCoin * poolBase;
	const newPoolBase = poolBase - value;
	const newPoolCoin = k / newPoolBase;
	return newPoolCoin - poolCoin;
}

	function createInputRow(labelText, placeholder, step = '0.000001') {
	...
	function renameOriginalLabel(amountInput) {
		const label = amountInput.closest('.space-y-2')?.querySelector('label');
		if (label) label.textContent = 'Value ($)';
	}

	function getTradeType(modal) {
		const title = modal.querySelector('[data-slot="dialog-title"]') || modal.querySelector('h2');
		const t = title?.textContent || '';
		return t.toUpperCase().includes('SELL') ? 'SELL' : 'BUY';
	}

	function getCoinSymbol(modal) {
		const title = modal.querySelector('[data-slot="dialog-title"]') || modal.querySelector('h2');
		const text = title?.textContent || '';
		const match = text.match(/(BUY|SELL)\s+([A-Z0-9]+)/i);
		return match ? match[2].toUpperCase() : null;
	}

	function getAmountInput(modal) {
		return modal.querySelector('input#amount');
	}
  ...
  function attachDualInputs(modal) {
	const amountInput = getAmountInput(modal);
	if (!amountInput) return;

	const tradeType = getTradeType(modal);
	const coinSymbol = getCoinSymbol(modal);
	if (!coinSymbol) return;

	const existingValue = modal.querySelector('[data-dual-input="value"]');
	const existingCoin = modal.querySelector('[data-dual-input="coin"]');

	if (existingValue || existingCoin) {
		const amountBlock = amountInput?.closest('.space-y-2');
		const coinLabel = `Coin Amount (${coinSymbol})`;

		if (tradeType === 'SELL') {
			if (amountBlock?.querySelector('label')) amountBlock.querySelector('label').textContent = coinLabel;
		} else {
			if (amountBlock?.querySelector('label')) amountBlock.querySelector('label').textContent = 'Value ($)';
		}

		const coinLabelEl = existingCoin?.querySelector('label');
		if (coinLabelEl) coinLabelEl.textContent = coinLabel;
	...
	renameOriginalLabel(amountInput);

	const coinLabel = `Coin Amount (${coinSymbol})`;

	const valueRow = createInputRow('Value ($)', '0.00', '0.000001');
	valueRow.wrapper.setAttribute('data-dual-input', 'value');

	const coinRow = createInputRow(coinLabel, `0.00 ${coinSymbol}`, '0.000001');
	coinRow.wrapper.setAttribute('data-dual-input', 'coin');

	// Place according to requirement
	if (tradeType === 'SELL') {
		amountBlock.querySelector('label').textContent = coinLabel;
		amountBlock.parentElement?.insertBefore(valueRow.wrapper, amountBlock);
	} else {
		amountBlock.parentElement?.insertBefore(coinRow.wrapper, amountBlock.nextSibling);
	}

	// Use a single value input ref that exists in the DOM
	const valueInput = tradeType === 'BUY' ? amountInput : valueRow.input;
...
const syncFromValue = async () => {
	...
	isSyncing = true;
	try {
		const pool = await getPoolInfo(coinSymbol);
		if (!pool) return;

		const value = sanitizeNumber(valueInput.value);
		if (tradeType === 'BUY') {
			const coins = ammEstimateCoinsFromValue(value, pool.coinAmount, pool.baseCurrencyAmount);
			coinRow.input.value = coins ? coins.toFixed(6) : '';
		} else {
			const coins = ammEstimateCoinsToSellForValue(value, pool.coinAmount, pool.baseCurrencyAmount);
			// IMPORTANT: update bound input via helper
			setInputValue(amountInput, coins ? coins.toFixed(6) : '');
		}
	} finally {
		isSyncing = false;
	}
...
const syncFromCoins = async () => {
	...
	try {
		...
		let coins = 0;
		if (tradeType === 'BUY') {
			coins = sanitizeNumber(coinRow.input.value);
			const value = ammEstimateValueFromCoins(coins, pool.coinAmount, pool.baseCurrencyAmount);
			// IMPORTANT: update bound input via helper
			setInputValue(valueInput, value ? value.toFixed(6) : '');
			setInputValue(amountInput, valueInput.value);
		} else {
			coins = sanitizeNumber(amountInput.value);
			const value = ammEstimateValueFromCoins(coins, pool.coinAmount, pool.baseCurrencyAmount);
			setInputValue(valueInput, value ? value.toFixed(6) : '');
		}
	} finally {
	...
	const maxButton =
		amountBlock.querySelector('button') ||
		amountBlock.parentElement?.querySelector('button');

if (maxButton && /max/i.test(maxButton.textContent || '')) {
	maxButton.addEventListener('click', () => {
		setTimeout(() => {
			if (tradeType === 'BUY') {
				setInputValue(valueInput, amountInput.value);
				syncFromValue();
			} else {
				syncFromCoins();
			}
	...
	if (tradeType === 'BUY') {
		coinRow.input.addEventListener('input', syncFromCoins);
	} else {
		amountInput.addEventListener('input', syncFromCoins);
	}

	if (tradeType === 'BUY') {
		if (valueInput.value) syncFromValue();
	} else {
		if (amountInput.value) syncFromCoins();
	}
}
	...
	const observer = new MutationObserver(() => {
		...
		if (modal && modal !== lastModal) {
			lastModal = modal;
			// delay to allow modal content to finish rendering
			setTimeout(() => attachDualInputs(modal), 50);
		}
	});
	...
	setTimeout(() => {
		const modal = document.querySelector('[data-slot="dialog-content"]') || document.querySelector('[role="dialog"]');
		if (modal) attachDualInputs(modal);
	}, 200);
}
